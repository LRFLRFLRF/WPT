...
class Maxwell:
    class coil_type(object):
        def __init__(self, coil, coil_p, coil_r):
            # 线圈名称列表  线圈加载面名称列表  线圈半径
            self.coil = coil
            self.coil_p = coil_p
            self.coil_r = coil_r
    ...
    def __init__(self, ):
        ...
        ################ 迭代参数
        # 线圈匝数范围   [下限， 上限， 步长]
        self.send_n_lim = [0, 0, 0]
        self.aux_n_lim = [0, 0, 0]
    ...
    def creat_coil_array(self, coil, coil_p, wire_r, coil_r, p_x, p_y, p_z, RelativeCS_name, zu_width, Dup_num_y,
                         Dup_num_x):
        ################################创建初始线圈 以及加载面
        # 选定坐标系
        self.SetWCS_maxwell("Global")
