...
%%两组发射线圈
send1 = send;   %一组四根导线的坐标
send1(:,1) = send1(:,1)+d+2*n;    %第二组导线坐标 
send = [send; send1;]; 

% 镜像法 镜像导线
send_c = send;         
send_c(:, 2) = send_c(:,2)*(-1);  %发射铁氧体的镜像导线坐标 
send_c2 = send;
send_c2(:,2) = send_c2(:,2)+2*h;   %接受铁氧体的镜像导线坐标
%原始的两组导线 I   发射铁氧体的镜像I'   原始两组导线I''   接受铁氧体镜像 I'  原始两组导线I''  
...
u = [u; u;];  %两组导体
u_c = u;
u_c(:,1) = u_c(:,1)*ur;  %I''电流在磁导率为ur*u0的介质里
u = [u; u; u_c; ];%u; u; u_c

D = sqrt((X-send(:,1)).^2 + (Y-send(:,2)).^2);   %xy 到 四个导线的距离矩阵
Bi = (1/(2*pi) * I(:,1).*(X-send(:,1))./D(:,1).^2).*u(:,1)   %每一个导线的B
B_sum = sum(Bi);     %B 的矢量和
