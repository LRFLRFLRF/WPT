...
% 带入变量
eq3_t = simplify(subs(eq3, [Lp*Cps, Ls*Css, L2*Csp], [temp2, temp3, temp4]));

% 求得a3_d1的解 并simplify化简
simp_eq3_t = simplify(solve(eq3_t, a3_d1),'IgnoreAnalyticConstraints', true, 'Steps', 10);

% 以a2_d1为自变量展开
a3_d1_res = collect(simp_eq3_t, a2_d1)
pretty(a3_d1_res)

% a3_d1_res中除去含有a2_d1因式剩余的部分   进行继续的以a3为自变量的展开
...
temp_a3_d1_res1 = simplify(subs(temp_a3_d1_res1, [sqrt(Csp)*sqrt(L2)], [1/w4]), 'Steps', 10)
